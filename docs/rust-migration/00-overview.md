...
- **Recommended next slice:** the plan/task command leaves and
  `NewPlan`/`PlanService` per 02-cli.md. `gw` was the last shared dependency
  those leaves needed — the model, conf, plan and gw layers are all ported
  now, so the remaining work is command wiring plus `main.rs` dispatch.
