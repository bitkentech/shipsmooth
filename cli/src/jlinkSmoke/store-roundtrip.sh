...
#
# Drives `store init --type filesystem` then `store info --json` through the REAL
...
# 1. Clean first run -> accept filesystem state at an explicit path.
INIT_OUT="$(cd "$REPO" && "$LAUNCHER" store init --type filesystem --path "$STATE" --json 2>&1)" \
