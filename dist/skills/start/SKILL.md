...
`[Local]` Script invocations use `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks <subcommand>`. All scripts read/write `.agents/plans/plan-{N}-tasks.xml` relative to the repo root.
...
ls .agents/plans/plan-{N}-tasks.xml   # if absent, run: /home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks init --plan {N} --tasks-from .agents/plans/plan-{N}.md

# 2. Review current task state
/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks show --plan {N}
...
`[Local]` All `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks` mutations also append one event to `.agents/ledger.jsonl` (content-addressed blobs in `.agents/objects/`). The XML remains the human-readable source of truth; the ledger is the machine-readable execution trace and the foundation for future parallel execution. Inspect with `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks ledger list` or `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks ledger verify`.
...
- `[Local]` Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks update-status --plan {N} --task {id} --status de-risked` and `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks add-comment --plan {N} --task {id} --message "De-risk draft ready for review"`.
...
- `[Local]` Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks update-status --plan {N} --task {id} --status agent-coded` and `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks set-commit --plan {N} --task {id} --commit $(git rev-parse HEAD)`.
...
4. Commit the completed task (tests + implementation):
   ...
   - `[Local]` Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks update-status --plan {N} --task {id} --status agent-coded` and `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks set-commit --plan {N} --task {id} --commit $(git rev-parse HEAD)`. No draft review needed.
...
- **Minor deviation** (task split, reorder, clarification):
  ...
  - `[Local]` Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks add-deviation --plan {N} --task {id} --type minor --message "..."`, continue.
- **Major deviation** (fundamental plan problem, architecture issue, blocked): Stop immediately.
  ...
  - `[Local]` Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks project-update --plan {N} --blocked --message "..."`.
...
- `[Local]` Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks project-update --plan {N} --status complete --message "Plan complete."`. Commit the final XML state. Update the permanent backlog feature issue (if tracked externally) or note delivery in the plan file.
...
- `[Local]` Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks update-status --plan {N} --task {id} --status needs-triage` for each unresolved task. Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks project-update --plan {N} --status in-review --message "..."`. Commit the XML. Wait for human to review.
...
- `[Local]` Run `/home/pramod/.cache/shipsmooth/runtime-0.3.1/bin/shipsmooth-tasks project-update --plan {N} --status abandoned --message "Superseded by plan-{M}."`. Commit the final XML state.
