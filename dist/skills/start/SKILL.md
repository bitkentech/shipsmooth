...
`[Local]` Script invocations use `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth <subcommand>`. All scripts read/write `.agents/plans/plan-{N}-tasks.xml` relative to the repo root.
...
```bash
${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan tag --plan {N} --kind version
# prints: git push origin plan-{N}-v{K}  — run that line to push
```

On clean completion:
```bash
${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan tag --plan {N} --kind complete
# prints: git push origin plan-{N}-complete
```

On abandonment:
```bash
${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan tag --plan {N} --kind abandoned
...
6. **Verify Preconditions:**
   ```bash
   ${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan preflight --plan {N}
...
```bash
${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan resume --plan {N}
...
```bash
${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan branch --issue {issue-id} --desc "{short-description}"
...
`[Local]` All `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth` mutations also append one event to `.agents/ledger.jsonl` (content-addressed blobs in `.agents/objects/`). The XML remains the human-readable source of truth; the ledger is the machine-readable execution trace and the foundation for future parallel execution. Inspect with `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth ledger list` or `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth ledger verify`.
...
- `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth task status --plan {N} --task {id} --status de-risked` and `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth task comment --plan {N} --task {id} --message "De-risk draft ready for review"`.
...
- `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth task status --plan {N} --task {id} --status agent-coded` and `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth task set-commit --plan {N} --task {id} --commit $(git rev-parse HEAD)`.
...
4. Commit the completed task (tests + implementation):
   ...
   - `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth task status --plan {N} --task {id} --status agent-coded` and `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth task set-commit --plan {N} --task {id} --commit $(git rev-parse HEAD)`. No draft review needed.
...
- **Minor deviation** (task split, reorder, clarification):
  ...
  - `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth task deviation --plan {N} --task {id} --type minor --message "..."`, continue.
- **Major deviation** (fundamental plan problem, architecture issue, blocked): Stop immediately.
  ...
  - `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan update --plan {N} --blocked --message "..."`.
...
- `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan update --plan {N} --status complete --message "Plan complete."`. Commit the final XML state. Update the permanent backlog feature issue (if tracked externally) or note delivery in the plan file.
...
- `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth task status --plan {N} --task {id} --status needs-triage` for each unresolved task. Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan update --plan {N} --status in-review --message "..."`. Commit the XML. Wait for human to review.
...
- `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.15/bin/shipsmooth plan update --plan {N} --status abandoned --message "Superseded by plan-{M}."`. Commit the final XML state.
