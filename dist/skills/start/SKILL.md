...
- `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.19/bin/shipsmooth task status --plan {N} --task {id} --status agent-coded` and `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.19/bin/shipsmooth task set-commit --plan {N} --task {id} --commit $(git rev-parse HEAD)`.
...
4. Commit the completed task (tests + implementation):
   ...
   - `[Local]` Run `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.19/bin/shipsmooth task status --plan {N} --task {id} --status agent-coded` and `${XDG_CACHE_HOME:-~/.cache}/shipsmooth/runtime-0.3.19/bin/shipsmooth task set-commit --plan {N} --task {id} --commit $(git rev-parse HEAD)`. No draft review needed.
